...
import com.aditya.parivarpocket.model.QuizDefinition;
import com.aditya.parivarpocket.model.QuizQuestion;
...
public class LearningModuleView extends VBox {
    ...
    private void showModuleModal(Lesson lesson) {
        ...
        // Footer with Quiz and Close buttons only
        ...
        Runnable updateQuestion = () -> {
            ...
            for (int i = 0; i < q.getOptions().size(); i++) {
                ...
                optContainer.setOnMouseClicked(e -> {
                    ...
                    // ---- Immediately disable all options and show loader ----
                    optionsList.getChildren().forEach(node -> node.setDisable(true));
                    feedbackBox.setVisible(false);

                    // Show a small loading indicator in the feedback area
                    ProgressIndicator loadingSpinner = new ProgressIndicator();
                    loadingSpinner.setPrefSize(28, 28);
                    Label checkingLabel = new Label("Checking answer...");
                    checkingLabel.setStyle("-fx-font-size: 13px; -fx-text-fill: #555;");
                    HBox loaderRow = new HBox(10, loadingSpinner, checkingLabel);
                    loaderRow.setAlignment(Pos.CENTER_LEFT);
                    loaderRow.setPadding(new Insets(6, 0, 0, 0));
                    feedbackBox.getChildren().add(loaderRow);
                    feedbackBox.setVisible(true);

                    // ---- Do network calls on background thread ----
                    javafx.concurrent.Task<Boolean> awardTask = new javafx.concurrent.Task<>() {
                        @Override
                        protected Boolean call() {
                            if (correct && !repository.isQuestionRewarded(user, q.getId())) {
                    ...
                    awardTask.setOnSucceeded(ev -> {
                        // Back on FX thread — update colours & feedback
                        feedbackBox.getChildren().remove(loaderRow);
                    ...
                    awardTask.setOnFailed(ev -> {
                        // Still update UI even if network failed
                        feedbackBox.getChildren().remove(loaderRow);
                        feedbackTitle.setText(correct ? "Correct \u2713" : "Incorrect \u2717");
                        feedbackTitle.setStyle("-fx-font-size: 15px; -fx-font-weight: bold; -fx-text-fill: " + (correct ? "#2e7d32" : "#c62828") + ";");
                        feedbackSub.setText(correct ? "Great job!" : "Better luck next time.");
                        feedbackBox.setVisible(true);
                        nextBtn.setDisable(false);
                    });

                    new Thread(awardTask).start();
                });
