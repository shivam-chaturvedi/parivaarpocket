...
import com.athena.parivarpocket.model.Alert;
...
public class NotificationsView extends VBox {
    ...
    private Node createAlertCard(Alert alert) {
        ...
        Label email = new Label(alert.getUserEmail());
        email.getStyleClass().add("alert-card-email");
        String metadataSummary = alert.getMetadataSummary();
        if (!metadataSummary.isBlank()) {
            Label metadata = new Label(metadataSummary);
            metadata.getStyleClass().add("alert-card-meta");
            text.getChildren().add(metadata);
        }
        Label time = new Label(relativeTime(alert.getCreatedAt()));
        time.getStyleClass().add("alert-card-time");
        text.getChildren().addAll(title, category, email, time);

        Region spacer = new Region();
        HBox.setHgrow(spacer, Priority.ALWAYS);

        Label dot = new Label("•");
        dot.getStyleClass().add("alert-card-dot");

        card.getChildren().addAll(icon, text, spacer, dot);
        return card;
    }
