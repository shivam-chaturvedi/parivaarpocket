...
import com.aditya.parivarpocket.model.LessonCompletion;
import com.aditya.parivarpocket.model.QuizAttempt;
import com.aditya.parivarpocket.model.StudentActivity;
import com.aditya.parivarpocket.model.StudentProfile;
import com.aditya.parivarpocket.model.StudentProgress;
...
public class EducatorDashboardView extends VBox {
    ...
    public EducatorDashboardView(DataRepository repository) {
        ...
        // Configure main content container (equivalent to the old root VBox)
        mainContent.setFillWidth(true);
        mainContent.setPadding(new Insets(24));
        mainContent.setStyle("-fx-background-color: white;");
        mainContent.getStyleClass().add("dashboard-container");
        ...
        mainContent.getChildren().addAll(monitoringSection, performanceSection);

        // Configure loading spinner
        loadingSpinner.setMaxSize(50, 50);
        loadingSpinner.setVisible(false);

        // Use a StackPane as the new root to layer the spinner over the content
        StackPane rootLayer = new StackPane(mainContent, loadingSpinner);
        
        // Ensure VBox settings apply to root to maintain layout
        setSpacing(0);
        setFillWidth(true);
    ...
    private HBox createStudentRow(StudentProfile profile) {
        ...
        StudentProgress progress = progressForProfile(profile);
        String rawName = progress != null ? progress.getStudentName() : null;
        String studentName = (rawName != null && !rawName.isBlank() && !rawName.equalsIgnoreCase("Student"))
                ? rawName
                : (profile.getEmail() != null && profile.getEmail().contains("@")
                        ? profile.getEmail().split("@")[0]
                        : (profile.getEmail() != null ? profile.getEmail() : "Student"));
    ...
    private void loadStudentProfiles() {
        ...
        mainContent.setOpacity(0.5); // Dim the background slightly
    ...
    private Node buildPerformanceSection() {
        ...
        nameCol.setCellFactory(col -> new TableCell<>() {
            ...
            protected void updateItem(StudentProfile profile, boolean empty) {
                ...
                VBox box = new VBox(4);
                String rawName = progress != null ? progress.getStudentName() : null;
                String displayName = (rawName != null && !rawName.isBlank() && !rawName.equalsIgnoreCase("Student"))
                        ? rawName
                        : (profile.getEmail() != null && profile.getEmail().contains("@")
                                ? profile.getEmail().split("@")[0]
                                : (profile.getEmail() != null ? profile.getEmail() : "Student"));
        ...
        // Check if columns are already added to avoid duplication on refresh
        if (studentTable.getColumns().isEmpty()) {
            studentTable.getColumns().addAll(nameCol, lessonsCol, quizCol, walletCol, jobCol, actionCol);
            
            // Fix columns so they cannot be reordered
            for (TableColumn<?, ?> col : studentTable.getColumns()) {
                col.setReorderable(false);
            }
        }
        
        // Ensure header background is white
        studentTable.getStylesheets().add(
            "data:text/css," + 
            ".table-view .column-header, .table-view .column-header-background, .table-view .filler, .table-view .column-header-background .filler {" +
    ...
    private void showStudentDetail(StudentProfile profile) {
        ...
        exportTab.setContent(buildExportTab(profile));
        exportTab.setClosable(false);

        tabs.getTabs().addAll(overviewTab, learningTab, jobTab, activityTab, financeTab, exportTab);
    ...
    private void handleExportForStudent(StudentProfile profile) {
        FileChooser fileChooser = new FileChooser();
        fileChooser.setTitle("Export Student Data");
        String safeName = displayStudentName(profile.getEmail()).replaceAll("[^a-zA-Z0-9.-]", "_");
        fileChooser.setInitialFileName(safeName + "_report.csv");
        fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("CSV Files", "*.csv"));
        File file = fileChooser.showSaveDialog(getScene().getWindow());
        ...
        if (file != null) {
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(file))) {
                ...
                Alert alert = new Alert(Alert.AlertType.INFORMATION, "Export successful for " + name + "!", ButtonType.OK);
                alert.showAndWait();
            } catch (IOException ex) {
                Alert alert = new Alert(Alert.AlertType.ERROR, "Failed to export: " + ex.getMessage(), ButtonType.OK);
                alert.showAndWait();
    ...
    private Node buildExportTab(StudentProfile profile) {
        VBox layout = new VBox(20);
        layout.setPadding(new Insets(24));
        layout.setAlignment(Pos.CENTER);

        Label title = new Label("Export Student Data");
        title.setStyle("-fx-font-size: 18px; -fx-font-weight: bold;");
        
        Label desc = new Label("Download a comprehensive CSV report containing all available metrics, learning progress, financial health, and job activities for this student.");
        desc.setWrapText(true);
        desc.setStyle("-fx-text-alignment: center; -fx-text-fill: #555;");

        Button exportBtn = new Button("Export This student data as csv");
        exportBtn.setStyle("-fx-padding: 12px 24px; -fx-font-size: 14px; -fx-background-color: #000; -fx-text-fill: white; -fx-cursor: hand; -fx-background-radius: 4;");
        exportBtn.setOnAction(e -> handleExportForStudent(profile));

        layout.getChildren().addAll(title, desc, exportBtn);
        return layout;
    }
    ...
    private HBox createTransactionRow(WalletEntry entry) {
        ...
        boolean isIncome = entry.getType() == com.aditya.parivarpocket.model.WalletEntryType.INCOME;
